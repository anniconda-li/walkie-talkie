...
static int app_ai_voice_ensure_buffer(void)
{
...
static int app_ai_voice_ensure_task(void)
{
    if (s_ai_task != NULL) {
        return 0;
    }

    if (app_ai_voice_ensure_buffer() != 0) {
        return -2;
    }
...
int app_ai_voice_start(void)
{
    ...
    int ret = app_ai_voice_ensure_task();
    if (ret != 0) {
        return ret;
    }
...
{
    if (!s_started || app_ai_voice_ensure_task() != 0) {
        return;
    }
