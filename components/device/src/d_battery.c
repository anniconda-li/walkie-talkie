...
static int d_battery_set_measurement_enabled(int enabled)
{
    int ret = d_battery_err_to_int(gpio_set_level(d_battery_ADC_EN_IO,
                                                  enabled != 0 ? 0 : 1));
    if (ret != 0) {
        D_LOGE(TAG, "电池采样使能控制失败, io=%d, ret=%d",
...
static int d_battery_gpio_init(void)
{
    ...
    int ret = d_battery_err_to_int(gpio_config(&en_gpio_cfg));
    if (ret != 0) {
        D_LOGE(TAG, "电池采样使能脚配置失败, io=%d, ret=%d",
               d_battery_ADC_EN_IO, ret);
        return ret;
    }

    ret = d_battery_set_measurement_enabled(0);
    if (ret != 0) {
...
int d_battery_deinit(void)
{
    ...
    (void)d_battery_set_measurement_enabled(0);
...
int d_battery_read_voltage_mv(int *voltage_mv)
{
    ...
    ret = d_battery_set_measurement_enabled(1);
    if (ret != 0) {
