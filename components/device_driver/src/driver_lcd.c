 ...
 * @brief LCD SPI 像素时钟。
 */
#define driver_lcd_SPI_PCLK_HZ (40u * 1000u * 1000u)
...
int driver_lcd_display_init(void)
{
    ...
    esp_lcd_panel_io_spi_config_t io_spi_cfg = {
        .dc_gpio_num = driver_lcd_DC_IO,
        .cs_gpio_num = driver_lcd_CS_IO,
        .pclk_hz = driver_lcd_SPI_PCLK_HZ,
        .lcd_cmd_bits = 8,
        .lcd_param_bits = 8,
        .spi_mode = 0,
        .trans_queue_depth = driver_lcd_SPI_TRANS_QUEUE_DEPTH,
    };

    ret = driver_lcd_err_to_int(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)bsp_spi_get_host(),
                                                      &io_spi_cfg,
                                                      &s_lcd_panel_io));
    if (ret != 0) {
    ...
    esp_lcd_panel_dev_config_t panel_cfg = {
        .reset_gpio_num = driver_lcd_RST_IO,
        .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_RGB,
        .bits_per_pixel = 16,
    };

    ret = driver_lcd_err_to_int(esp_lcd_new_panel_st7789(s_lcd_panel_io,
                                                      &panel_cfg,
                                                      &s_lcd_panel));
    if (ret != 0) {
    ...
    ret = driver_lcd_err_to_int(esp_lcd_panel_reset(s_lcd_panel));
    if (ret == 0) {
        ret = driver_lcd_err_to_int(esp_lcd_panel_init(s_lcd_panel));
    }
    if (ret == 0) {
        ret = driver_lcd_err_to_int(esp_lcd_panel_invert_color(s_lcd_panel, false));
    }
    if (ret == 0) {
        ret = driver_lcd_err_to_int(esp_lcd_panel_swap_xy(s_lcd_panel, driver_lcd_SWAP_XY != 0));
    }
    if (ret == 0) {
        ret = driver_lcd_err_to_int(esp_lcd_panel_mirror(s_lcd_panel,
                                                      driver_lcd_MIRROR_X != 0,
                                                      driver_lcd_MIRROR_Y != 0));
    }
    if (ret == 0) {
        ret = driver_lcd_err_to_int(esp_lcd_panel_set_gap(s_lcd_panel, 0, 0));
    }
    if (ret == 0) {
...
int driver_lcd_touch_init(void)
{
    ...
    esp_lcd_touch_config_t touch_cfg = {
        .x_max = driver_lcd_H_RES,
        .y_max = driver_lcd_V_RES,
        .rst_gpio_num = driver_lcd_TOUCH_RST_IO,
        .int_gpio_num = driver_lcd_TOUCH_INT_IO,
        .levels = {
            .reset = 0,
            .interrupt = 0,
        },
        .flags = {
            .swap_xy = driver_lcd_SWAP_XY != 0,
            .mirror_x = driver_lcd_MIRROR_X != 0,
            .mirror_y = driver_lcd_MIRROR_Y != 0,
        },
    ...
    ret = driver_lcd_err_to_int(esp_lcd_touch_new_i2c_ft5x06(s_lcd_touch_io,
                                                          &touch_cfg,
                                                          &s_lcd_touch));
    if (ret != 0) {
...
{
    int ret = driver_lcd_display_init();
    if (ret != 0) {
        return ret;
    }

    ret = driver_lcd_touch_init();
    if (ret != 0) {
        driver_lcd_deinit();
        return ret;
    }
...
int driver_lcd_deinit(void)
{
    ...
    if (s_lcd_touch != NULL) {
        int del_ret = driver_lcd_err_to_int(esp_lcd_touch_del(s_lcd_touch));
        s_lcd_touch = NULL;
        if (ret == 0) {
            ret = del_ret;
        }
    ...
    if (s_lcd_touch_io != NULL) {
        int del_ret = driver_lcd_err_to_int(esp_lcd_panel_io_del(s_lcd_touch_io));
        s_lcd_touch_io = NULL;
        if (ret == 0) {
            ret = del_ret;
        }
    ...
    if (s_lcd_panel != NULL) {
        int del_ret = driver_lcd_err_to_int(esp_lcd_panel_del(s_lcd_panel));
        s_lcd_panel = NULL;
        if (ret == 0) {
            ret = del_ret;
        }
    ...
    if (s_lcd_panel_io != NULL) {
        int del_ret = driver_lcd_err_to_int(esp_lcd_panel_io_del(s_lcd_panel_io));
        s_lcd_panel_io = NULL;
        if (ret == 0) {
            ret = del_ret;
        }
...
int driver_lcd_fill_screen(uint16_t color)
{
    ...
    uint16_t line[driver_lcd_H_RES];
    for (uint32_t i = 0; i < driver_lcd_H_RES; i++) {
        line[i] = color;
    }

    for (uint32_t y = 0; y < driver_lcd_V_RES; y++) {
        int ret = driver_lcd_err_to_int(esp_lcd_panel_draw_bitmap(s_lcd_panel,
                                                               0,
                                                               (int)y,
                                                               (int)driver_lcd_H_RES,
                                                               (int)y + 1,
                                                               line));
        if (ret != 0) {
                       ...
                       uint8_t *point_num)
{
    ...
    esp_lcd_touch_point_data_t touch_data[driver_lcd_TOUCH_MAX_POINTS] = {0};
    uint8_t read_max = max_points;
    if (read_max > driver_lcd_TOUCH_MAX_POINTS) {
        read_max = driver_lcd_TOUCH_MAX_POINTS;
    }
    ...
    ret = driver_lcd_err_to_int(esp_lcd_touch_get_data(s_lcd_touch,
                                                    touch_data,
                                                    point_num,
                                                    read_max));
    if (ret != 0) {
